/* holidayfyi-embed v1.0.0 | MIT | https://widget.holidayfyi.com */
...
function countdownToDate(isoDate) {
  const target = /* @__PURE__ */ new Date(isoDate + "T00:00:00");
  const now = /* @__PURE__ */ new Date();
  const totalMs = target.getTime() - now.getTime();
  return msToCountdown(totalMs);
}
...
function initTodayTool(el, config) {
  const shadow = createShadow(el, config);
  const container = createWidgetRoot(shadow, el, "utilityfyi-today-widget");
  renderLoading(container);
  fetchAPI(config.apiBase, "today/").then((data) => {
    var _a, _b;
    const holidays = (_b = (_a = data.results) != null ? _a : data.holidays) != null ? _b : [];
    if (holidays.length === 0) {
      container.innerHTML = `
          <div class="utilityfyi-header">
            <div>
              <div class="utilityfyi-header-title">Today's Holidays</div>
              <div class="utilityfyi-header-subtitle">${(/* @__PURE__ */ new Date()).toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })}</div>
            </div>
          </div>
          <div style="padding:20px 18px;text-align:center;color:var(--muted);font-size:0.9rem;">No holidays today</div>
          ${poweredByHTML(config)}
        `;
      return;
    }
    const items = holidays.map((h) => {
      var _a2, _b2, _c;
      const emoji = (_a2 = h.emoji) != null ? _a2 : "\u{1F389}";
      const type = ((_b2 = h.type) != null ? _b2 : "").toLowerCase();
      const typeColors = (_c = TYPE_COLORS2[type]) != null ? _c : { bg: "#F1F5F9", fg: "#374151" };
      const url = `https://${config.domain}/holiday/${esc(h.slug)}/`;
      return `
          <a href="${url}" target="_blank" rel="noopener" style="display:flex;align-items:center;gap:10px;padding:8px 18px;text-decoration:none;color:var(--text);border-bottom:1px solid var(--border);transition:background 0.1s;">
            <span style="font-size:1.2rem;flex-shrink:0;">${emoji}</span>
            <div style="flex:1;min-width:0;">
              <div style="font-size:0.875rem;font-weight:500;">${esc(h.name)}</div>
              <div style="font-size:0.75rem;color:var(--muted);">
                ${h.country_name ? esc(h.country_name) + " \xB7 " : ""}${type ? badge(type.charAt(0).toUpperCase() + type.slice(1), typeColors.bg, typeColors.fg) : ""}
              </div>
            </div>
          </a>
        `;
    ...
    container.innerHTML = `
        <div class="utilityfyi-header">
          <div>
            <div class="utilityfyi-header-title">Today's Holidays</div>
            <div class="utilityfyi-header-subtitle">${holidays.length} holiday${holidays.length !== 1 ? "s" : ""} \xB7 ${(/* @__PURE__ */ new Date()).toLocaleDateString("en-US", { month: "long", day: "numeric" })}</div>
          </div>
        </div>
        ${items}
        <div class="utilityfyi-view-link"><a href="https://${config.domain}/today/" target="_blank" rel="noopener">All today's holidays ${externalLinkIcon}</a></div>
        ${poweredByHTML(config)}
      `;
  }).catch(() => {
    renderError(container, "Unable to load today's holidays.", config);
  });
}

// src/tools/upcoming.ts
function initUpcomingTool(el, config) {
  var _a;
  const dataset = el.dataset;
  const limit = (_a = dataset.limit) != null ? _a : "5";
  const shadow = createShadow(el, config);
  const container = createWidgetRoot(shadow, el, "utilityfyi-upcoming-widget");
  renderLoading(container);
  fetchAPI(config.apiBase, `upcoming/`, { limit }).then((data) => {
    var _a2, _b;
    const holidays = (_b = (_a2 = data.results) != null ? _a2 : data.holidays) != null ? _b : [];
    if (holidays.length === 0) {
      container.innerHTML = `
          <div class="utilityfyi-header">
            <div><div class="utilityfyi-header-title">Upcoming Holidays</div></div>
          </div>
          <div style="padding:20px 18px;text-align:center;color:var(--muted);font-size:0.9rem;">No upcoming holidays found</div>
          ${poweredByHTML(config)}
        `;
      return;
    }
    const items = holidays.map((h) => {
      var _a3, _b2, _c;
      const emoji = (_a3 = h.emoji) != null ? _a3 : "\u{1F4C5}";
      const dateStr = (_c = (_b2 = h.next_date) != null ? _b2 : h.date) != null ? _c : "";
      const cd = dateStr ? countdownToDate(dateStr) : null;
      const daysLabel = cd ? cd.isPast ? "Today" : `in ${cd.days} day${cd.days !== 1 ? "s" : ""}` : "";
      const url = `https://${config.domain}/holiday/${esc(h.slug)}/`;
      return `
          <a href="${url}" target="_blank" rel="noopener" style="display:flex;align-items:center;gap:10px;padding:8px 18px;text-decoration:none;color:var(--text);border-bottom:1px solid var(--border);transition:background 0.1s;">
            <span style="font-size:1.2rem;flex-shrink:0;">${emoji}</span>
            <div style="flex:1;min-width:0;">
              <div style="font-size:0.875rem;font-weight:500;">${esc(h.name)}</div>
              <div style="font-size:0.75rem;color:var(--muted);">${esc(dateStr)}</div>
            </div>
            ${daysLabel ? `<span class="utilityfyi-badge" style="flex-shrink:0;">${esc(daysLabel)}</span>` : ""}
          </a>
        `;
    }).join("");
    container.innerHTML = `
        <div class="utilityfyi-header">
          <div>
            <div class="utilityfyi-header-title">Upcoming Holidays</div>
            <div class="utilityfyi-header-subtitle">Next ${holidays.length} holidays</div>
          </div>
        </div>
        ${items}
        <div class="utilityfyi-view-link"><a href="https://${config.domain}/" target="_blank" rel="noopener">All holidays on ${esc(config.name)} ${externalLinkIcon}</a></div>
        ${poweredByHTML(config)}
      `;
  }).catch(() => {
    renderError(container, "Unable to load upcoming holidays.", config);
  });
}
...
// src/tools/countdown.ts
var CountdownWidget = class extends LiveWidget {
  constructor(container, config, month, day, name, emoji) {
    super();
  ...
  render() {
    const cd = daysUntil(this.month, this.day);
    if (cd.isPast || cd.days === 0 && cd.hours === 0 && cd.minutes === 0) {
      return `
        <div class="utilityfyi-header">
          <div class="utilityfyi-img" style="font-size:32px;">${this.emoji}</div>
          <div>
            <div class="utilityfyi-header-title">${esc(this.name)}</div>
            <div class="utilityfyi-header-subtitle">Today!</div>
          </div>
        </div>
        <div class="utilityfyi-result-display">
          <div class="utilityfyi-result-big" style="font-size:2rem;">\u{1F389} Today!</div>
        </div>
    ...
    return `
      <div class="utilityfyi-header">
        ...
        <div>
          <div class="utilityfyi-header-title">${esc(this.name)}</div>
          <div class="utilityfyi-header-subtitle">Countdown</div>
        </div>
      </div>
      <div class="utilityfyi-countdown">
        <div class="utilityfyi-countdown-seg">
          <div class="utilityfyi-countdown-num">${cd.days}</div>
          <div class="utilityfyi-countdown-label">Days</div>
        </div>
        <div class="utilityfyi-countdown-seg">
          <div class="utilityfyi-countdown-num">${pad2(cd.hours)}</div>
          <div class="utilityfyi-countdown-label">Hours</div>
        </div>
        <div class="utilityfyi-countdown-seg">
          <div class="utilityfyi-countdown-num">${pad2(cd.minutes)}</div>
          <div class="utilityfyi-countdown-label">Min</div>
        </div>
        <div class="utilityfyi-countdown-seg">
          <div class="utilityfyi-countdown-num">${pad2(cd.seconds)}</div>
          <div class="utilityfyi-countdown-label">Sec</div>
        </div>
      </div>
      ${poweredByHTML(this.config)}
    `;
...
function initCountdownTool(el, config) {
  var _a, _b, _c, _d;
  const dataset = el.dataset;
  const month = parseInt((_a = dataset.month) != null ? _a : "12", 10);
  const day = parseInt((_b = dataset.day) != null ? _b : "25", 10);
  const name = (_c = dataset.name) != null ? _c : "Countdown";
  const emoji = (_d = dataset.emoji) != null ? _d : "\u{1F389}";
  if (isNaN(month) || isNaN(day) || month < 1 || month > 12 || day < 1 || day > 31) {
    const shadow2 = createShadow(el, config);
    const container2 = createWidgetRoot(shadow2, el, "utilityfyi-countdown-widget");
    container2.innerHTML = `<div class="utilityfyi-error"><p>Invalid date. Use data-month (1-12) and data-day (1-31).</p></div>${poweredByHTML(config)}`;
    return;
  }
  const shadow = createShadow(el, config);
  const container = createWidgetRoot(shadow, el, "utilityfyi-countdown-widget");
  const widget = new CountdownWidget(container, config, month, day, name, emoji);
...
function initHolidayBadgeInline(el, config) {
  var _a, _b, _c;
  const dataset = el.dataset;
  const type = ((_a = dataset.type) != null ? _a : "").toLowerCase();
  const emoji = (_b = dataset.emoji) != null ? _b : "";
  const shadow = createShadow(el, config);
  const hostStyle = document.createElement("style");
  hostStyle.textContent = `:host { display: inline-flex; vertical-align: baseline; }`;
  shadow.appendChild(hostStyle);
  const wrapper = document.createElement("span");
  const colors = (_c = TYPE_STYLES[type]) != null ? _c : { bg: "#F1F5F9", fg: "#374151" };
  const label = type ? type.charAt(0).toUpperCase() + type.slice(1) + " Holiday" : "Holiday";
  wrapper.innerHTML = `<span style="display:inline-flex;align-items:center;gap:3px;padding:1px 6px;border-radius:10px;background:${colors.bg};color:${colors.fg};font-size:0.75rem;font-weight:600;line-height:1.4;white-space:nowrap;">${emoji ? emoji + " " : ""}${escapeHTML(label)}</span>`;
  shadow.appendChild(wrapper);
}

// src/_entry_holidayfyi.ts
function initWidget(el, type, config) {
  ...
  switch (type) {
    ...
    case "faq":
      ...
      break;
    case "today":
      initTodayTool(el, config);
      break;
    case "upcoming":
      initUpcomingTool(el, config);
      break;
    case "countdown":
      initCountdownTool(el, config);
      break;
    case "badge":
      initHolidayBadgeInline(el, config);
      break;
    default:
      break;
  }
...
(function bootstrap() {
  const config = '{"site":"holidayfyi","name":"HolidayFYI","domain":"holidayfyi.com","accent":"#EAB308","attribute":"data-holidayfyi","apiBase":"https://holidayfyi.com/api/v1/","searchPath":"/search/","entityName":"Holidays","entitySlug":"holidays"}';
...
function makeWidgetElement(widgetType, initFn, domainAttrs) {
  ...
  return class extends HTMLElement {
    ...
    connectedCallback() {
      ...
      this._syncDataAttrs();
      initFn(this, '{"site":"holidayfyi","name":"HolidayFYI","domain":"holidayfyi.com","accent":"#EAB308","attribute":"data-holidayfyi","apiBase":"https://holidayfyi.com/api/v1/","searchPath":"/search/","entityName":"Holidays","entitySlug":"holidays"}');
    }
    ...
    _syncDataAttrs() {
      const attrKey = '{"site":"holidayfyi","name":"HolidayFYI","domain":"holidayfyi.com","accent":"#EAB308","attribute":"data-holidayfyi","apiBase":"https://holidayfyi.com/api/v1/","searchPath":"/search/","entityName":"Holidays","entitySlug":"holidays"}'.attribute.replace("data-", "");
...
(function registerElements() {
  ...
  const site = '{"site":"holidayfyi","name":"HolidayFYI","domain":"holidayfyi.com","accent":"#EAB308","attribute":"data-holidayfyi","apiBase":"https://holidayfyi.com/api/v1/","searchPath":"/search/","entityName":"Holidays","entitySlug":"holidays"}'.site;
  const defs = [
    ...
    [`${site}-today`, initTodayTool, ["country"]],
    [`${site}-upcoming`, initUpcomingTool, ["limit", "country"]],
    [`${site}-countdown`, initCountdownTool, ["month", "day", "name", "emoji"]]
  ];
